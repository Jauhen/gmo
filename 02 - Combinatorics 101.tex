...
\begin{frame}{Rule of division}
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      ...
      The rule of division also works in the case when we need to put $5$ fruits not in a line, but in a circle. In this case, these $5$ permutations are the same:
      \vspace*{-0.1cm}
      \begin{nscenter}
        \begin{tikzpicture}[outer sep=0, inner sep=0]
          \node (plate1) at (0, 0)
      ...
      \end{nscenter}
      \vspace*{-0.2cm}
      so the total number of ways to put $5$ elements in a circle is $\frac{5!}{5} = 24$.
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Permutations with identical elements}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      Now get back to the problem from Kindergarten
...
\begin{frame}{And beyond}
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{enumerate}[a)]
        \item This problem may be solved using \highlightbox{permutations with identical elements}, which we covered in these slides, (every spoon may be put in $4$ different cups), so the answer is $4^7 = 16,384$.
        \item This problem may be solved using a technique called \highlightbox{stars and bars}, we will cover it in a future lesson, calculated as $\dbinom{13}{3} = 286$.
        ...
        \item Here we need to calculate the number of \highlightbox{Young tableaux} with $10$ cells and $4$ boxes, which is far beyond middle school math. For this particular problem it is easier to just do the \highlightbox{\emph{\textbf{case}}\ \ \emph{\textbf{bashing}}} (head down and write out all cases) to find out that answer is $23$.  We’ll practice case bashing in a future lesson as well.
      \end{enumerate}
      }
    \end{column}
  \end{columns}
